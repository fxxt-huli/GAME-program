...
var Game = {
    ...
    'init': function() {
        setTimeout(function() {
            Game.box.style.background = 'url(images/fengmian1.jpg)';
            var oTitle = Game.ctE('h3');
            oTitle.innerHTML = '空军壹号';
            var oDiv = Game.ctE('div');
            oDiv.innerHTML = '<p class="ac">简单模式</p><p class="ac">一般模式</p><p class="ac">困难模式</p>';
    ...
    'plane': function(ev, t, c) {
        ...
        var bT = Game.box.offsetTop 
               + Game.oPlane.clientHeight / 2 
               + (Game.box.offsetHeight 
               - Game.getstyle(Game.box, 'height')) / 2;

        var bL = Game.box.offsetLeft 
               + Game.oPlane.clientWidth / 2 
               + (Game.box.offsetWidth 
               - Game.getstyle(Game.box, 'width')) / 2;
        ...
        var topMax = Game.getstyle(Game.box, 'height') 
                    - Game.oPlane.clientHeight / 2;

        var leftMin = -Game.oPlane.clientWidth / 2;

        var leftMax = Game.getstyle(Game.box, 'width') 
                    - Game.oPlane.clientWidth / 2;
    ...
    'enemy': function() {
        ...
        var left = Math.random() * 
                (Game.box.clientWidth - Game.getstyle(oEnemy, 'width') / 2);
    ...
    'getstyle': function(obj, attr) {
        return obj.currentstyle ? 
            parseInt(obj.currentstyle[attr]) : parseInt(getComputedStyle(obj)[attr]);
    }
};